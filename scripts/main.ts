...
async function uploadReferenceImages(
  ...
  // Use CDP file chooser interception: click the attach button, intercept the
  // file chooser event, and provide files through CDP — no native OS dialog.
  await cdp.send('Page.setInterceptFileChooserDialog', { enabled: true }, { sessionId });

  // Set up a one-shot handler to provide files when the chooser opens
  ...
  // Click the attach/upload button to trigger the file chooser
  console.error('[grok] Clicking attach button...');
  const attachClicked = await evaluate<boolean>(cdp, sessionId, `
    (() => {
      // Look for the attach/plus button near the input area
      ...
      // Fallback: find button with a plus/attach SVG icon near the input
      ...
      for (const inp of inputs) {
        ...
        for (let i = 0; i < 5 && container; i++) {
          ...
          for (const b of btns) {
            // The attach button typically has an SVG and is before the text input
  ...
  if (!attachClicked) {
    // Fallback: directly find and use the file input
    console.error('[grok] Attach button not found, setting file input directly...');
    ...
    if (fileInputObjId) {
      ...
      throw new Error('Could not find file input or attach button in Grok UI.');
    }
  } else {
    // Wait for file chooser to be intercepted and files provided
    await Promise.race([fileChooserPromise, sleep(5000)]);
    console.error('[grok] Files provided via file chooser interception');
  }

  // Wait for the UI to process the uploaded files
  await sleep(3000);
